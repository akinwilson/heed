...
STR_TO_MODEL_CFGS = {"HSTAT": cfg.HTSwin(),"ResNet":cfg.ResNet(),"DeepSpeech": cfg.DeepSpeech() , "LeeNet": cfg.LeeNet(), "MobileNet": cfg.MobileNet()}
STR_TO_MODLES = {"HSTAT": HTSwinTransformer,"ResNet":ResNet,"DeepSpeech": DeepSpeech, "LeeNet": LeeNet, "MobileNet": MobileNet }
...
class Fitter:
    ...
    def __call__(self):
        ...
        trainer = Trainer(accelerator="gpu",devices=3,strategy='dp',sync_batchnorm = True,logger = logger, max_epochs = self.cfg_fitting.max_epoch, callbacks=self.callback_list(),
                          num_sanity_val_steps = 2, resume_from_checkpoint = self.cfg_fitting.resume_from_checkpoint, gradient_clip_val=1.0,fast_dev_run= self.cfg_fitting.fast_dev_run)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_name', type=str, default= MODEL_NAMES[MODEL_NAMES.index("ResNet")])
    ...
    # init the exporter.
    onnx_exporter = OnnxExporter(model=predictor,
