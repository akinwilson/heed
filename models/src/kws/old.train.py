...
from kws.Architecture import Architecture
from kws.layer import AugmentationManager
from kws.eval import Metric
from kws.plot import Plotter
from kws.util import OnnxExporter
from kws.config import DataPath
...
def main(cfg):
    ...
    ):  # resources for distributed data parallel training available
    ...
    scheduler = optim.lr_scheduler.StepLR(
        optimizer, step_size=20, gamma=cfg.fit_param["gamma"]
    )
    ...
    for epoch in range(1, cfg.fit_param["max_epochs"] + 1):
        ...
        model, total_train_epoch_losss, train_acc, train_ttr, train_ftr = (
        ...
        model, total_val_epoch_losss, val_acc, val_ttr, val_ftr = validate_one_epoch(
            model, val_loader, cfg
        )
        val_loss = total_val_epoch_losss / len(val_loader)
        val_losses.append((epoch, val_loss))
        metrics = {"val_acc": val_acc, "val_ttr": val_ttr, "val_ftr": val_ftr}
        ...
        early_stopping_condition_met, extrema_val = check_es(
            es_metric_history, es_patience=cfg.fit_param["es_patience"]
        )
