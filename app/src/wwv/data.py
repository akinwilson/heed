...
class Padder:
    ...
    def __call__(self, x: torch.tensor) -> torch.tensor:
        padding = torch.tensor([0.0]).repeat(
            [1, self.cfg.max_sample_len - x.size()[-1]]
        )
        x_new = torch.hstack([x, padding])
        x_new = x_new.to(device)
        assert (
            x_new.size()[-1] == self.cfg.max_sample_len
        ), f"Incorrect padded length, Should be {self.cfg.max_sample_len}, got {x_new.size()[-1]}"
        return x_new  # (1 ,1 , pad_to_len)


class AudioDataset(Dataset):
    def __init__(self, df_path, cfg_model, cfg_feature, cfg_fitting):

        if cfg_fitting.dev_run:
            # using 10 % of available training data
            frac = float(os.getenv("FAST_DEV_RUN_FRACTION", 0.1))
            logger.info(f"Using fraction {frac:.2f} of dataset ... ")
            self.df = pd.read_csv(df_path, on_bad_lines="skip").sample(
                frac=frac,
                ignore_index=True,
            )
            self.df = self.df[self.df["label"].notna()]
            print(f"len of DF {len(self.df)}")
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg_model.audio_feature == "mfcc":
            x = self.x_mfcc(x)
            n_mfcc = int(
                (self.cfg_feature.sample_rate * self.cfg_feature.audio_duration)
                / self.cfg_feature.window_step
            )
            x = x[:, :, :n_mfcc]
