...
class InferenceResult(BaseModel):
    ...
    """
    wake_word_probability: float = Field(..., example=0.987526, title='Probablity of wakeword')
    prediction: float = Field(..., example=1.0, title='Predicted label')
    false_alarm_probability: float = Field(... , example=0.001, title="False alarm probability")
