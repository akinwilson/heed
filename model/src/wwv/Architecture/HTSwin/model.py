...
# Use same implementation to allow using pretrained model weights from research paper 
...
class HTSwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.freq_ratio = self.spec_size // self.cfg_signal.mel_bins
        self.interpolate_ratio = 32     # Downsampled ratio
        # Spectrogram extractor
        window = 'hann'
        center = True
        pad_mode = 'reflect'
        self.spectrogram_extractor = Spectrogram(n_fft=self.cfg_signal.window_size,
                                                 hop_length=self.cfg_signal.hop_size, 
                                                 win_length=self.cfg_signal.window_size,
                                                 window=window,
                                                 center=center,
                                                 pad_mode=pad_mode, 
        ...
        amin = 1e-10
        top_db = None
        self.logmel_extractor = LogmelFilterBank(sr=self.cfg_signal.sample_rate,
                                                  n_fft=self.cfg_signal.window_size, 
                                                 n_mels=self.cfg_signal.mel_bins,
                                                 fmin=self.cfg_signal.fmin,
                                                 fmax=self.cfg_signal.fmax,
                                                 ref=ref,
                                                 amin=amin,
                                                 top_db=top_db, 
                                                freeze_parameters=True)
